...
- 场景 `{"type":"scene","name","outcome":"PASS|FAIL|TIMEOUT|ENV_FAIL","ticks","wallMs","reason"}`，
  可选 `"skipped":true`（v0 附录，2026-08-06）。跳过的场景 outcome 是 **PASS**——它不是失败——
  所以 outcome 分不出"跑了并通过"和"没跑"。此前唯一的信号是 `reason` 的 `"skipped: "` 前缀，
  一个两个写入方各自记着、每个读者各自解析散文的契约。**只在 true 时出现**，所以没有跳过的
  一轮逐字节不变；也正因如此，读者必须**同时**认前缀：字段缺席既可能是"没跳过"也可能是
  "旧文件"，只认字段会把旧文件里的每一次跳过当成跑过了——两个信号各自失效的方向都是**假绿**，
  但失效的场合不同，所以两个都读。
- 尾 `{"type":"done","scenes":N}`（缺尾 = harness 中途死亡 = RED）
- `canary` ∈ NONE | MUST_FAIL | MUST_TIMEOUT | MUST_SWALLOW | MUST_SKIP；
  MUST_SWALLOW 场景**不得**有场景记录（有 = 门死）。
  MUST_SKIP 场景**必须**有场景记录且必须是跳过（**执行了 = 门死 = DEAD**）：这类场景的被测对象
  就是"缺席会被判为跳过"，而跳过记的是 PASS，所以在此之前它们无论跳过还是执行都是绿的——
  全套里唯一不可能失败的场景。判 DEAD 而不是 RED 的理由同 MUST_SWALLOW：坏掉的是**所有**
  一致性套件的跳过所依赖的那套缺席判定，这一轮的绿因此不再是证据。
  金丝雀场景若记录为 `ENV_FAIL`（例如 PREP 阶段区块加载失败）同样按 outcome
  不匹配处理 → exit 2 DEAD；这是保守裁决——即便根因是环境问题，整轮结果仍作废。

### 跨轮覆盖门（v0 附录，2026-08-06）
单轮判决**不**因跳过变红，这是对的：dedicated 拓扑没有玩家，不是需要玩家的场景的缺陷。
但同一个理由让跳过在**所有**轮里都不可见——一套场景若在它跑过的每个拓扑上都跳过，
报出来是一个从未执行过该主题的绿。ATM10 就是这个形状：`atm.advancementUnlocksForAPlayer`
（进度解锁）和 `atm.aQuestCanBeClaimed`（任务领取）注册了、对过账、进了尾计数，
而在存在过的每一轮里都因为没有玩家跳过。每一轮都诚实地绿。

不变量：**任一轮注册的场景，必须在至少一轮里执行过。** 这条**不是**人工维护的
"哪条该在哪跑"清单——那种清单要由刚忘了加它的人来更新。这条不需要维护：
新增一个场景，它当场就被这条门覆盖；新增一个拓扑，门只会更容易满足。
唯一豁免在场景自己身上声明（`@SceneDef(mustSkip = true)`），而那是断言不是免责。

Gradle `stagewrightCoverage` / CLI `--coverage <files>`。故意**不**依赖各拓扑的 run 任务：
一个判红的拓扑会抛异常中断构建，而那正是这份跨轮报告最有话说的时候。缺结果文件按 ENV 报，
不静默略过——略过会缩小"执行过"的并集，把某一轮没跑变成对跑过的那几轮的指控。
...
## 心跳文件 `stagewright-progress.json`（v0 附录，2026-08-10）
结果文件回答"这一轮怎么样"，只有写完了才回答得了。**缺尾 = harness 中途死亡 = RED**
这条判得对，但它说不出任何下一步需要的东西：死在哪个场景、当时世界还在不在跑、
离被杀还有多久。运维接着要做的是打开几十 MB 的游戏日志往回找最后一个场景名——
这是机器本该记下来的事。

`<runDir>/stagewright-progress.json`，UTF-8，**单行 JSON 对象，每次覆写**（不是 JSONL）：

```json
{"type":"progress","scene":"wd.journey09Iron","ticks":41234,"ticksInWindow":3,"windowMs":90000,"atMs":1786350908936}
```

- 由游戏内 stall watchdog 线程每秒写一次，**健康轮也写**——它要顶用的那次死亡，
  正是这个线程观察不到的那种（编排器墙钟 kill、OOM、别人的 `halt`、GC 螺旋）。
- **覆写而非追加**：任意长度的一轮只占常量磁盘，读的一方也不用扫——文件里有什么，
  就是最后为真的东西。反过来说，它**不进结果文件**：那份文件是本轮的判定清单，
  一秒一条心跳在门禁轮里是 150 行、长套件里数千行，足以把人打开文件要读的记录淹掉。
- `ticksInWindow`/`windowMs` 是**原始两数**而不是速率：除完之后"世界停了"和
  "世界在爬"都约等于 0，而这两个是不同的 bug，要去进程的不同半边找。
- **provision 会连同结果文件一起删**。留下来的心跳比没有心跳更坏——它照样报得出
  一个场景名，只不过是上一轮的那个。
- 裁决**只在缺尾时**读它。心跳活得比一轮健康的运行更久（只覆写、不删除），
  正常 RED（有尾、某个场景断言失败）时打出来指的是毫不相干的最后一个场景，
  会把调查引向完全错误的方向。
- 写失败一律吞掉。这与 `ResultsJsonl` 写失败即抛的规矩相反，理由也相反：那份文件
  载着判决，写不下去就不许显示为绿；这份文件不载判决，不许让一轮本来没事的运行
  因为磁盘满而结束。
- 本节新增一份**独立于结果文件线协议**的描述性文件（不改 `stagewright-results.jsonl`
  格式、不改任何退出码含义），契约仍冻结在 v0，不升版。
